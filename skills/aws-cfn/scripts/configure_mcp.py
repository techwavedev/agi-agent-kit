#!/usr/bin/env python3
"""Configure MCP client with AWS CloudFormation server."""
...
def main():
    ...
    config = {"command": "uvx", "args": ["awslabs.cfn-mcp-server@latest"],
              "env": {"AWS_PROFILE": args.profile, "AWS_REGION": region}}
    
    existing = json.loads(cfg_path.read_text()) if cfg_path.exists() else {}
    existing.setdefault("mcpServers", {})["cloudformation"] = config
    
    print(json.dumps({"mcpServers": {"cloudformation": config}}, indent=2))
    if not args.dry_run:
