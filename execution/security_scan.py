...
def scan_dependencies() -> dict:
    ...
    if not npm_result.get("available", False):
        ...
        if npm_result.get("reason", "").startswith("npm") and "not installed" not in npm_result.get("reason", ""):
...
def scan_blocked_packages() -> dict:
    ...
    for path in ROOT_DIR.rglob("*"):
        ...
        rel_path = str(path.relative_to(ROOT_DIR))
        if rel_path == "execution/security_scan.py" or rel_path == "templates/base/execution/security_scan.py":
            continue
